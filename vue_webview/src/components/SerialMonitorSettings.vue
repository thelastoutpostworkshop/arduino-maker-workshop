...
<template>
    ...
    <!-- Line Ending -->
    <v-select :disabled="!store.boardConnected?.detected_ports" v-model="props.monitorPortSettings.lineEnding"
        :items="lineEndings" item-title="title" item-value="value" density="compact" label="Line Ending" class="mb-3"
        hide-details />

    <!-- Data Bits -->
    <v-select :disabled="!store.boardConnected?.detected_ports" v-model="props.monitorPortSettings.dataBits"
        :items="[5, 6, 7, 8]" density="compact" label="Data Bits" class="mb-3" hide-details />

    <!-- Stop Bits -->
    <v-select :disabled="!store.boardConnected?.detected_ports" v-model="props.monitorPortSettings.stopBits"
        :items="stopBits" item-title="title" item-value="value" density="compact" label="Stop Bits" class="mb-3"
        hide-details />

    <!-- Parity -->
    <v-select :disabled="!store.boardConnected?.detected_ports" v-model="props.monitorPortSettings.parity"
        :items="parity" item-title="title" item-value="value" density="compact" label="Parity" hide-details />
</template>
